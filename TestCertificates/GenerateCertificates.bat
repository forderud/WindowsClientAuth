...
makecert.exe -ic TestRootCertificate.cer -iv TestRootCertificate.pvk -pe -sv ClientCert.pvk -a sha256 -n "CN=ClientCert" -len 2048 -b 01/01/2022 -e 01/01/2032 -sky exchange ClientCert.cer -eku 1.3.6.1.5.5.7.3.2,1.3.6.1.5.5.7.3.3
:: Bundle to .pfx
...
openssl.exe x509 -req -extfile openssl.ini -extensions MyCustomExtensions -CA TestRootCertificate.pem -CAcreateserial -in localhost.csr -out localhost.crt -days 3650
